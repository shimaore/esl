...
We start one server
        ...
        service = seem ->
          ...
          switch destination
            when  'answer-wait-3020'
              ...
              yield @command('hangup').catch -> true
      ...
      it 'should handle one call', (done) ->
          ...
          ev.on 'server7002', ->
            client.end -> Promise.delay(3500).then -> done()
...
These tests are long-runners.
    ...
    describe 'The client', ->
        ...
        before (done) ->
          ...
          server = FS.server -> service.call(this)
          server.on 'listening', ->
            done()
          server.listen 7000
          null
...
The first successful route is selected.
            ...
            switch destination
              ...
              when  'answer-wait-3050'
                yield @command 'answer'
                yield Promise.delay 3050
                yield @command('hangup').catch -> true

              when 'server7022'
                yield @command 'set', 'a=2'
                yield @command 'set', 'b=3'
                yield @command 'set', 'c=4'
                yield @exit()
                ev.emit 'server7022'
