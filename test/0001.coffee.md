      ...
      describe 'when it is started', ->
        ...
        it 'should reloadxml with keepConnected', (done) ->
          ...
          client = FS.client ->
            ...
            .then -> client.end()
            .then done
          .keepConnected client_port, '127.0.0.1'
...
The goal is to document how to detect error conditions, especially wrt LCR conditions.
      ...
      before (done) ->
        ...
        service = seem ->
          ...
          switch
            when destination is 'answer-wait-3000'
              yield @command 'answer'
              yield Promise.delay 3000

            when destination is 'wait-24000-ring-ready'
              yield Promise.delay 24000
              yield @command 'ring_ready'
              yield Promise.delay 9999

            when m = destination.match /^wait-(\d+)-respond-(\d+)$/
              yield Promise.delay parseInt m[1]
              yield @command 'respond', m[2]
              yield Promise.delay 9999

            when destination is 'foobared'
              yield @command 'respond', 400
