        ...
        service = seem ->
          ...
          switch destination
            when  'answer-wait-3020'
              ...
              yield delay 3000
              yield @command('hangup').catch -> debug 'caught in answer-wait-3020'

            when 'server7002'
              res = yield @command 'answer'
              res.body['Channel-Call-State'].should.eql 'ACTIVE'
              yield @command('hangup').catch -> debug 'caught in server7002'
              ev.emit 'server7002'

            when 'server7003'
              res = yield @command 'answer'
              res.body['Channel-Call-State'].should.eql 'ACTIVE'
              yield @command('hangup').catch -> debug 'caught in server7003'
              ev.emit 'server7003'
            ...
            when 'server7008'
              ...
              yield @command 'answer'
              yield delay 1000
              yield @command('hangup').catch -> debug 'caught'
        ...
        service = seem ->
          ...
          switch
            ...
            when destination is 'wait-24000-ring-ready'
              yield delay 24000
              yield @command('ring_ready').catch -> true
              yield delay 9999

            when m = destination.match /^wait-(\d+)-respond-(\d+)$/
              yield delay parseInt m[1]
